...
public class DialogUtils {
    ...
    public static void displayDialogSettings(final Context context, final String message) {
        ...
        alertDialog.setMessage(message);
        alertDialog.setButton(AlertDialog.BUTTON_NEGATIVE, context.getString(R.string.cancel), (dialog, which) -> {
            context.startActivity(new Intent(android.provider.Settings.ACTION_SECURITY_SETTINGS));
            dialog.dismiss();
        });
        alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, context.getString(R.string.settings), (dialog, which) -> dialog.dismiss());
